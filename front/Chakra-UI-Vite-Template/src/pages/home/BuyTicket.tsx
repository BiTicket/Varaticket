import React, { useState } from "react";
import { useAccount, useApi, useAlert } from "@gear-js/react-hooks";
import { web3FromSource } from "@polkadot/extension-dapp";
import { ProgramMetadata } from "@gear-js/api";
import {
  Box,
  Button,
  FormControl,
  FormLabel,
  Input,
} from '@chakra-ui/react';


function BuyTicket() {
  const alert = useAlert();
  const { accounts, account } = useAccount();
  const { api } = useApi();

  const [formData, setFormData] = useState({
    creator: '',
    name: '',
    description: '',
    number_of_Tickets: 0,
    date: 0
  });

  interface MyMetaData {
    title: string;
    description :string;
    media: string;
    reference: string
}

var obj: MyMetaData = { "title": "","description": "","media": "","reference": ""};

console.log([obj]);

  const [creator, setCreator] = useState<string>('0x78ddba5083a4a93e53d63e9624d65635e824fbcf14d67a33f7a7f1882317f737');
  const [event_id, setEventId] = useState<number>(8);
  const [amount, setAmount] = useState<number>(3);
  const [metadata, setTheMetaData] = useState<MyMetaData[]>([obj]);
  
  //  console.log(themetadata)

  // Add your programID
  const programIDFT = "0x7fb00a6f016d49675e4a0ff34fe1f4ab7e30912e1aa499de42c7c8c5fb4544da";

  // Add your metadata.txt
  const meta =
   "";

  const metadataprogram = ProgramMetadata.from(meta);

  const handleCreatorChange = (e: React.ChangeEvent<HTMLInputElement>) => setCreator(account?.address || '0x78ddba5083a4a93e53d63e9624d65635e824fbcf14d67a33f7a7f1882317f737');
  const handleEventIDChange = (e: React.ChangeEvent<HTMLInputElement>) => setEventId(parseInt(e.target.value));
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => setAmount(parseInt(e.target.value, 10));
  //const handleMetaDataChange = (e: React.ChangeEvent<HTMLInputElement>) => setTheMetaData(e.target.value);

  const message = {
    destination: programIDFT, // programId
    payload: { buytickets: { creator, event_id, amount, metadata } },
    gasLimit: 98998192450,
    value: 0,
  };

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData({
      ...formData,
      [name]: value
    });
  };

  const signer = async (e:any) => {
    e.preventDefault();
    setTheMetaData([obj])
    const localaccount = account?.address;
    const isVisibleAccount = accounts.some((visibleAccount) => visibleAccount.address === localaccount);

    if (isVisibleAccount) {
      try {
        // Create a message extrinsic
        const transferExtrinsic = await api.message.send(message, metadataprogram);
        console.log(message);
        const injector = await web3FromSource(accounts[0].meta.source);

        transferExtrinsic
          .signAndSend(
            account?.address ?? alert.error("No account"),
            { signer: injector.signer },
            ({ status }) => {
              if (status.isInBlock) {
                alert.success(`Transaction included in block: ${status.asInBlock.toString()}`);
              } else {
                console.log("In Process", status);
                if (status.isFinalized) {
                  alert.success(`Transaction finalized: ${status.asFinalized.toString()}`);
                }
              }
            }
          )
          .catch((error: any) => {
            console.error("Transaction failed", error);
            alert.error("Transaction failed");
          });
      } catch (error) {
        console.error("Error creating message extrinsic:", error);
      }
    } else {
      alert.error("Account not available to sign");
    }
  };

  return (
    <Box as="form" onSubmit={signer} p={4} borderWidth={1} borderRadius="lg">
      {/* <FormControl mb={4}>
        <FormLabel>ID</FormLabel>
        <Input
          placeholder='Creator'
          name='creator'
          value={formData.creator}
          onChange={handleChange}
        />
      </FormControl> */}
      <FormControl mb={4}>
        <FormLabel>Creator</FormLabel>
        <Input
          placeholder='Creator'
          name='creator'
          value={creator}
          onChange={handleCreatorChange}
        />
      </FormControl>
      <FormControl mb={4}>
        <FormLabel>Event Id</FormLabel>
        <Input
          placeholder='EventID'
          name='eventid'
          value={event_id}
          onChange={handleEventIDChange}
        />
      </FormControl>
      <FormControl mb={4}>
        <FormLabel>Amount of tickets</FormLabel>
        <Input
          type='number'
          placeholder='Number of Tickets'
          name='amount'
          value={amount}
          onChange={handleAmountChange}
        />
      </FormControl>
      {/* <FormControl mb={4}>
        <FormLabel>Metadata</FormLabel>
        <Input
          placeholder='metadata'
          name='themetadata'
          value={themetadata}
          onChange={handleMetaDataChange}
        />
      </FormControl> */}
      <Button type='submit' colorScheme="blue">Submit</Button>
    </Box>
    
  );
}

export { BuyTicket };
