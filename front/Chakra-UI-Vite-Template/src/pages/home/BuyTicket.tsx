...
function BuyTicket() {
  ...
  const [event_id, setEventId] = useState<number>(8);
  const [amount, setAmount] = useState<number>(3);
  const [metadata, setTheMetaData] = useState<MyMetaData[]>([]);
  ...
  const handleCreatorChange = (e: React.ChangeEvent<HTMLInputElement>) => setCreator(account?.decodedAddress || '');
  const handleEventIDChange = (e: React.ChangeEvent<HTMLInputElement>) => setEventId(parseInt(e.target.value));
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => setAmount(parseInt(e.target.value, 10));
  //const handleMetaDataChange = (e: React.ChangeEvent<HTMLInputElement>) => setTheMetaData(e.target.value);
  ...
  const signer = async (e:any) => {
    e.preventDefault();
    // setTheMetaData(newMsg)

    let ticket = {
      "title": "Sum 41 concert in Madrid 26/08/2022",
      "description": "Sum 41 Madrid 26/08/2022 Ticket. Row 4. Seat 4.",
      "media": "sum41.com",
      "reference": "UNKNOWN"
    };

    const multitokenMetadata = metadataProgram.createType(7, [ticket, ticket]);
    console.log(multitokenMetadata.toJSON());

    const message = {
      destination: programIDFT, // programId
      payload: { buytickets: { creator, event_id, amount, metadata: multitokenMetadata } },
